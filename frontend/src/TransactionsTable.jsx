    ...
    () => [
      ...
      { 
        ...
        cell: info => {
          ...
          return (
            <a href={`https://polygonscan.com/tx/${v}`} target="_blank" rel="noopener noreferrer" style={{ color: '#1976d2', textDecoration: 'underline' }}>
              {short}
            </a>
          );
        }
                ...
                {table.getRowModel().rows.map(row => (
                    <TableRow key={row.id} sx={{ borderBottom: 'none' }}>
                        {row.getVisibleCells().map(cell => (
                            <TableCell key={cell.id} align="left" sx={{ fontSize: '12px', borderBottom: 'none' }}>
